  ...
  $.each(users, function(_i, usr) {
    ...
    html += `<td><code>${usr.status}</code></td>`;
      ...
      if (usr.status !== "BANNED") {
          html += `<button onclick="cwos.faderBox.url('/app/developer/patch/user', {code: ${usr.code}, server: '${usr.server}', callback: 'requery'}, {exitBtn:true});" type="button" class="theme-button mini blue no-shadow"><i class="fas fa-edit"></i> Edit</button>`;
  ...
  $.each(apps, function(_i, app) {
    html += "<tr>";
      ...
      if (app.status !== "BANNED") {
          html += `<button onclick="cwos.faderBox.url('/app/developer/patch/app', {name: '${app.name}', server: '${app.server}', callback: 'requery'}, {exitBtn:true});" type="button" class="theme-button mini blue no-shadow"><i class="fas fa-edit"></i> Edit</button>`;
        ...
        html += `<button onclick="clipboardCopy('${app.puKey}');" type="button" class="theme-button mini asphalt no-shadow"><i class="fas fa-copy"></i> Public Key</button>`;
      html += `</td>`;
    html += "</tr>";
  });
  $(`${conf.container}`).html(html);
...
function lsHst (histories) {
  ...
  $.each(histories, function(_i, hst) {
    html += "<tr>";
      ...
      html += `<td>`;
        html += `<button onclick='clipboardCopy("${encodeURI(hst.param)}");' type="button" class="theme-button mini asphalt no-shadow"><i class="fas fa-copy"></i> Request Param</button>`;
      html += `</td>`;
